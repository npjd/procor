...
task("deploy:procor", "Deploy a Procor contract")
    ...
    .setAction(async ({ logs, treeDepth, verifierAddress }, { ethers }) => {
        ...
        const FactoryContract = await ethers.getContractFactory("Procor", {
            libraries: {
                IncrementalBinaryTree: incrementalBinaryTreeLib.address
            }
        })

        const contract = await FactoryContract.deploy( verifierAddress)

        await contract.deployed()

        if (logs) {
            console.info(`Procor contract has been deployed to: ${contract.address}`)
        }
